...
friend.Base = new Person("KazeDevID", "Akaeru Bot");
friend.Friends = new Person("Fardan", "Fax Bot");
friend.Friends2 = new Person("Haris Ganz", "HanzBotz");
friend.Friends3 = new Person("Ridho", "Atak Bot");
...
// OTHER
    ...
    //ANTI CALL
    conn.ev.on('call', async (toll) => {
    let botNumber = await conn.decodeJid(conn.user.id)
    let asuuu = JSON.parse(fs.readFileSync('./metadata/setting.json'));
    if (!asuuu.setting.anticall) return
    console.log(toll)
    for (let konn of toll) {
    if (konn.isGroup == false) {
    if (konn.status == "offer") {
    let pa7rick = await conn.sendTextWithMentions(konn.from, `*${NameBot}* tidak bisa menerima panggilan ${konn.isVideo ? `video` : `suara`}. Maaf @${konn.from.split('@')[0]} kamu akan diblockir. Jika tidak sengaja silahkan hubungi Owner untuk dibuka !`)
    conn.sendContact(konn.from, global.owner, pa7rick)
    await sleep(8000)
    await conn.updateBlockStatus(konn.from, "block")
    }
    }
    }
    })
